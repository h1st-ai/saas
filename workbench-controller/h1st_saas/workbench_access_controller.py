...
class WorkbenchAccessController:
    ...
    def add_shares(self, shares):
        for s in shares:
            if s.get('permission'):
                self.share(s['workbench_id'], s['user_id'], s.get('permission'), { 'first_name': s.get('first_name'), 'last_name': s.get('last_name') })
            else:
    ...
    def share(self, workbench_id, user_id, permission=None, extras = {}):
        """
        ...
        dyn.put_item(
            ...
            Item={
                ...
                'user_id': {'S': user_id},
                'first_name': {'S': extras.get('first_name')},
                'last_name': {'S': extras.get('last_name')},
